...
last_modified_at: 2024/10/07 18:01:48
---

<div class="message">
Llama 3와 3.1 모델이 얼마만큼 변화가 있는지 비교한다.
</div>
...
Llama 3와 3.1 모델이 얼마만큼 차이가 있는지 비교한다. 그 전에 먼저 Qwen2와 아키텍처 차이를 확인해본다.

# 모델 구조 비교
llama 3 8B 구조. 이후 버전인 3.1, 3.2도 모두 동일한 구조를 갖고 있다.
...
Qwen2 7B 구조. 2.5도 동일한 구조를 갖고 있다.
...
Vocab Size 뿐만 아니라 아키텍처의 사이즈도 많이 다르며, Qwen2의 경우 MLP 레이어에 bias도 갖고 있다.

## 라마 3.2 1B까지 차이

GPT-2 이후 라마 2부터 변화 과정을 도식화. 라마 3에서 GQA가 모든 모델에 적용된 것 외에는 계속 동일하다.
...
Llama 3 8B와 Llama 3.1 8B의 변화율이 몇 퍼센트인지 도식화하면 다음과 같다.
...
주로 input에 가까운 레이어의 변화가 크며, Q의 첫 번째 레이어는 40% 가까운 변화를 보인다. input_layernorm과 post_attention_layernorm은 뒤로 갈수록 2% 내외로 변화가 줄어들며 전반적으로 큰 차이가 없다. 그러나 MLP를 비롯한 나머지 모든 레이어는 대부분 20% 내외의 변화가 있다.

파라미터의 변화를 추출하는 코드는 [(비공개) #1-14 `model-diff.py`](/wiki/Private-Links/) 참고

## 파라미터 설명
위에서 언급한 각 파라미터의 위치는 다음과 같다.
...
좀 더 보기 좋게 다시 정리하면 다음과 같다.

<img src="https://lh3.googleusercontent.com/pw/AP1GczOr8S_3_JGIpBx8zByZaJP0DEBebH3bPv63UtmZaqdS6KgMMQ3dGOITnyggkeisWGh2qFRjPOxY-LO52W6xapfBcEUfG3CaTpIuu3oBIVTmPPXPy_e2yeMBgMJSvXQd-Ir5q6s0yuyBbAR3qFs92ApCDw=w922-h941-s-no-gm?authuser=0" width="70%">