...
layout: wiki 
title: Serverless AI
last-modified: 2021/02/27 18:03:03
---
...
아직 한국은 지원하지 않아서 모델을 한국 이외 리전에 업로드해야 한다. input-type이 json인데, 특이한 형태로 받고 있어서 `gcloud ai-platform predict --help` 도움말로 확인 필요. new line으로 입력값을 구분한다. 바로 모델을 업로드 하니 잘 안되는데, `local predict` 기능이 있어서 여기서 먼저 잘 되는지 확인할 수 있다. 매우 편리. 실제로 로컬 모듈을 활용하기 때문에, tf도 설치되어 있어야 한다.
